...
int hadoop_rpc_connect(struct connection_state * state, const char * host, const uint16_t port)
{
  ...
  if(connect(state->sockfd, (struct sockaddr *) &state->servaddr, sizeof(state->servaddr)) < 0)
  {
    goto fail;
  }
  if(hadoop_rpc_send(state, &header, sizeof(header)) < 0)
  {
    goto fail;
  }
  ...
  if(hadoop_rpc_send_out_of_band(state, buf, len, CONNECTION_CONTEXT_CALL_ID) < 0)
  {
    goto fail;
  }  
